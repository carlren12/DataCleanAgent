...
# 数据文件
DATA_FILE      = "/data/users/renhao/datasets/med_79w/data_test_1k.csv"
OUTPUT_FILE    = "/data/users/renhao/experiment/results_1k_concurrent.json"
LOG_FILE       = "/data/users/renhao/experiment/run_1k_concurrent.log"
SAVE_EVERY     = 50
MAX_SAMPLES    = 1000
SC_ROUNDS      = 3
MAX_WORKERS     = 8   # 外层并发数
...
def load_data():
    rows = []
    with open(DATA_FILE, "r", encoding="utf-8-sig") as f:  # utf-8-sig 去除BOM
...
def main():
    ...
    log("=" * 60)
    log("DataCleanAgent 并发版 (ThreadPoolExecutor 8线程)")
    log(f"数据: {DATA_FILE}")
